...
speech = gTTS(text=content, lang='en', tld='com', slow=False)
...
# Load the music file
music_clip = AudioFileClip("gameplay/music.mp3")

if (audio_clip.duration + 1.3 > 58):
    print('\nSpeech too long!\n' + str(audio_clip.duration) + ' seconds\n' + str(audio_clip.duration + 1.3) + ' total')
    exit()

# Ensure music_clip is not longer than audio_clip
if music_clip.duration > audio_clip.duration:
    # Generate a random start point for music_clip
    random_start = random.uniform(0, music_clip.duration - audio_clip.duration)
    music_clip = music_clip.subclip(random_start, random_start + audio_clip.duration)

# Decrease the volume of music_clip
music_clip = music_clip.volumex(0.5)  # Adjust the volume level (0.5 means half of the original volume)

print('\n')

# Trim a random part of minecraft gameplay
